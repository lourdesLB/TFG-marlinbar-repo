---
title: "Preprocesamiento del dataset de sarcoma (GSE21050)"
...
microarray_platform <- "hgu133plus2.db"  # plataforma de microarray (para sumarizacion)
not_gene_columns <- c("group","time_survivor")           # columas que no son sondas

# Acciones a realizar
do_log2 <- "YES"   # Estos datos ya se encuentran en intensidades log2
do_norm <- "NO"  # Estos datos no presentan a penas variabilidad experimental
...
# Intensity plot
log2_xlim_before <- c(-1,15)
log2_ylim_before <- c(0,0.5)
log2_xlim_after <- c(-1,15)
log2_ylim_after <- c(0,0.5)

# Volcanoplot
volcano_legend <- "bottomleft"
volcano_cex <- 0.5
volcano_xlim <- c(-2,2)
volcano_ylim <- c(0,10)
...
groupv0 <- "metastasis-no"    # nombre de las clases etiquetadas como 0 y 1
...
dataset1 <- read_csv(paste0(dataset_files_folder_extraction,
                            dataset_name,"_1_notnulldata.csv"), 
...
write_csv(dataset2, paste0(dataset_files_folder_extraction,
...
```{r}
# Almacenar en csv
write_csv(dataset3, paste0(dataset_files_folder_extraction,
...
# La variabilidad experimental es baja, las distribuciones por cuantiles son uniformes. En este caso no necesitamos aplicar normalizacion por cuantiles
```
...
if (do_norm=="YES"){
  ...
  plot(density(unlist(gene_matrix4[1,]), na.rm = TRUE), 
     main = paste(dataset_name, "- probes intensities (after preprocess)"), 
...
```{r}
# Almacenar en csv
write_csv(dataset4, paste0(dataset_files_folder_extraction,
                           dataset_name,"_4_normcuantiles.csv")
          )

write_csv(dataset4[,-which(names(dataset4) %in% not_gene_columns)],
          paste0(dataset_files_folder_extraction,
...
write.table(gene_symbols, 
            file = paste0(dataset_files_folder_extraction,
